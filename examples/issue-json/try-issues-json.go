...
func main() {
	...
	paras := &redmine4go.IssueListParameter{"0", "3", "", ""}

	issueList, error := c.GetIssueListOfProject(projectId, paras, nil)
	if error == nil {
		fmt.Printf("Number of issues = %d\n", issueList.TotalCount)
		fmt.Printf("issue 1 - Project = %s\n", issueList.Issues[2].Project.Name)
		fmt.Printf("issue 1 - ID = %d\n", issueList.Issues[2].ID)
		fmt.Printf("issue 1 - Subject = %s\n", issueList.Issues[2].Subject)
		fmt.Printf("issue 1 - Status = %s\n", issueList.Issues[2].Status.Name)
		fmt.Printf("issue 1 - Author = %s\n", issueList.Issues[2].Author.Name)
		fmt.Printf("issue 1 - Assigned To = %s\n", issueList.Issues[2].AssignedTo.Name)
	} else {
		fmt.Printf("%s\n", error)
	}
